...
export default function InstructorDashboard() {
  ...
  // Redirect to instructor login if not authenticated or not instructor/admin
  useEffect(() => {
    if (!isLoading && (!isAuthenticated || !["instructor", "admin"].includes(user?.role || ""))) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <GraduationCap className="h-8 w-8 animate-spin mx-auto mb-2 text-green-600" />
          <p>Loading...</p>
        </div>
      </div>
    );
